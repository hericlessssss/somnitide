...
- **Hierarquia Visual no Card do Relógio**: A estrutura do `.hero-card` na Home foi reordenada para um fluxo lógico ideal (Hora -> Ação Primária -> Feedback Secundário). O botão de iniciar/parar sessão foi movido para o meio da tela, logo abaixo do relógio de formato monumental, enquanto os textos de feedback como "Nenhuma sessão ativa" ou "Sessão em andamento" foram deslocados para o rodapé do card.
