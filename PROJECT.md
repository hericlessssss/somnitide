...
- **Refinamento de Tipografia e Adensamento**: Atendendo à dificuldade de escala relatada, o `font-size` global do `.page-title` foi forçado com `!important` para o dobro do tamanho base (`clamp(4.0rem, 10.4vw, 5.5rem)`). O espaçamento entre título e subtítulo foi reduzido para um valor negativo (`-2px`) para colá-los visualmente, conforme o print de referência. A distância para o conteúdo foi reduzida para o mínimo (`8px` / `16px`), aproximando o parágrafo explicativo.
- **Substituição de Tipografia de Corpo (Body Text)**: Após análise detalhada de design (UI/UX), a fonte global do corpo do texto (`--font-main`) foi alterada de **Plus Jakarta Sans** para **Inter**. A Inter é reconhecida como padrão de mercado (SaaS, Apple, Figma) por sua extrema legibilidade na tela e elegância em tamanhos menores. A fonte monumental Plus Jakarta Sans foi restrita exclusivamente aos cabeçalhos `.page-title`. Adicionalmente, foi aplicado um `letter-spacing: -0.015em` global e ampliado o `line-height` para `1.6`, resultando em parágrafos mais densos, esteticamente apurados e harmônicos, garantindo alto padrão visual sem comprometer a clareza de leitura.
- **Refinamento do Perfil Público (Layout & Dados)**: A tela de perfil público (`public-profile.component.ts`) apresentava quebra de respiro (margin collapse) entre blocos no mobile, cobrindo o conteúdo final. Isso foi corrigido integrando o componente `<app-page-container>`, que aplica o distanciamento padrão da barra de navegação base.
  No bloco de "Resumo de Atividade", a métrica estática de jornada foi substituída por dados dinâmicos: Posição Global (`rankPosition`) e Última Atividade (calculada em tempo real como "Há X min/horas" a partir de `updatedAtUtc`), eliminando valores que pareciam "travados" e aumentando a percepção de vivacidade do perfil.
- **Refatoração Premium & Social Footer (Design Inteligente)**: As telas de autenticação e o perfil público passaram por uma refatoração estética de alto nível.
  - **Auth Screens**: Introduzido um efeito de "Ambient Light" (soft radial gradients) e glassmorphism refinado, além de um rodapé discreto com links para LinkedIn e GitHub do desenvolvedor. O layout foi harmonizado com maior respiro vertical (`space-2xl`) e uma borda divisória no rodapé.
- **Padronização de Branding & Layout Core**: 
  - **Identidade somnitide**: Realizada a atualização global da marca para letras minúsculas ("somnitide") em todos os headers, títulos de páginas, documentação e metatags, garantindo uma identidade visual coesa e moderna.
  - **Correção de Alinhamento Auth**: Corrigido o erro de sintaxe CSS nos componentes de Login e Registro que causava o desalinhamento do link "Crie uma conta". O rodapé agora está perfeitamente centralizado e harmonizado com o grid premium.
- **Perfil Público Dashboard**: O layout foi reorganizado em um grid inteligente inspirado em dashboards premium. Foi adicionada uma barra de navegação dedicada (`chevron_left`), o título "Perfil do Usuário" e o alinhamento de ícones de cabeçalho foi corrigido para perfeita simetria com os títulos. A seção "Insights do Sono" agora exibe métricas reais e dinâmicas: o **Score Médio** considera os últimos 30 dias para maior representatividade, a **Última Noite** captura a sessão mais recente independente do período, e a **Sequência (Streak)** informa a consistência atual. O rodapé de texto redundante foi removido para um visual mais limpo, e o sistema agora diferencia "falta de dados" (exibindo `--`) de scores zerados.
- **Refinamento de Branding (Login/Registro)**: As telas de autenticação receberam um ajuste estético para maior minimalismo. O nome da marca foi alterado para letras minúsculas ("somnitide"), o texto de apoio "Métricas do seu sono" foi removido, e a logo foi reposicionada acima do título, criando uma hierarquia visual vertical mais limpa e focada no ícone da marca.
- **Correção de Autofill "Nuclear" (Login/Registro)**: A abordagem anterior falhou em algumas versões do Chrome. Implementada uma solução mais agressiva no `styles.css` utilizando `-webkit-background-clip: text` em conjunto com `box-shadow` e cores hardcoded (`#0F1622`). Esta técnica "desliga" o renderizador de destaque nativo do browser, forçando o campo a manter a estética Dark SaaS da aplicação mesmo quando preenchido automaticamente, garantindo legibilidade total do e-mail e da senha.
- **Top 3 Ranking Mobile**: Resolvido o problema de alinhamento dos 3 melhores jogadores na versão mobile da página de Ranking. A classe `.podium-section` foi ajustada no `@media (max-width: 768px)` com `align-items: stretch` e limite idêntico aos containers subsequentes (`max-width: 600px`), permitindo que os cards de pódio ocupem simetricamente a largura da tela conforme as demais listagens como "Lendários".
- **Nova Identidade Visual Dinâmica**: O ícone provisório em Material Icons (`<mat-icon>waves</mat-icon>`) foi universalmente substituído por uma nova marca própria, renderizada via tag `<img>` em `src/assets/logo.png`. A substituição ocorreu na NavBar do layout autenticado, bem como nas páginas de Login e de Registro. O novo Favicon do app também foi padronizado para herdar esta mesma identidade gráfica refinada (`favicon.png`).
- **Science Brief Discreto**: A fonte do parágrafo explicativo na Home foi reduzida para `0.78rem` com opacidade `0.7`, tornando-o um elemento de apoio discreto que não compete com o título monumental ou com o relógio.
- **Feedback Visual Avançado em Mobile Nav**: Corrigido um bug na barra de navegação inferior onde a classe `.active` não surtia efeito devido à ausência de uma variável de cor e configurações de rota. Implementado um state design explícito (`color: var(--color-primary) !important`) acoplado a uma micro-interação no ícone (`transform: translateY(-2px) scale(1.1)`) e um soft glow (`drop-shadow` de 8px). Adicionado `[routerLinkActiveOptions]="{exact: true}"` nas âncoras para garantir que o framework Angular aplique a classe ativa corretamente. Isso fornece clareza imediata sobre a página atual sem quebrar o estilo _glassmorphism_ do componente.
- **Micro-ajustes de Espaçamento e Tipografia**: Atendendo ao refinamento visual, a distância entre títulos e subtítulos foi reduzida para o mínimo (`--page-header-gap: 2px`). O recuo vertical entre o cabeçalho da página e o conteúdo principal foi calibrado para "uma linha" (`--page-content-gap: 16px`). Na `Home`, a seção explicativa (Science Brief) teve sua tipografia suavizada (tamanho `0.82rem` e opacidade reduzida) para garantir que o destaque permaneça no relógio e nas ações principais, mantendo a sobriedade do design.
- **Harmonização do Perfil**: Seguindo os princípios de adensamento da Home, os textos auxiliares do Profile ("Este é o seu nome único...", "Soma de toda a sua jornada", "membro desde...") tiveram suas tipografias reduzidas (entre `9px` e `10px`) e opacidades rebaixadas (`0.4` a `0.5`). Foi necessário usar selector override global (`::ng-deep .mat-mdc-form-field-hint`) para sobrepor os hint mappings padrão do Material Design e garantir a discrição exigida (cor `text-muted`).
